            ...
            <div class="space-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        @error('nama') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="nis" class="block text-sm font-medium text-gray-700 mb-1">NIS <span class="text-red-500">*</span></label>
                        <input type="text" name="nis" id="nis" value="{{ old('nis') }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        @error('nis') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" x-data="{ tingkatan: '{{ old('tingkatan', 'tsanawiyah') }}', getKelasOptions() { if(this.tingkatan === 'tsanawiyah') return ['7', '8', '9']; if(this.tingkatan === 'aliyah') return ['10', '11', '12']; return ['Takhassus']; } }">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="kelas" id="kelas" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                            <template x-for="opt in getKelasOptions()" :key="opt">
                                <option :value="opt" x-text="opt" :selected="opt == '{{ old('kelas') }}'"></option>
                            </template>
                        </select>
                    </div>
                    <div>
                        <label for="tanggal_lahir" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Lahir</label>
                        <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                    </div>
                </div>
                <div>
                    ...
                    <select name="wali_murid_id" id="wali_murid_id" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        ...
                        @foreach($waliMuridList as $wali)
                            <option value="{{ $wali->id }}" {{ old('wali_murid_id') == $wali->id ? 'selected' : '' }}>{{ $wali->name }} ({{ $wali->email }})</option>
                        @endforeach
                    </select>
                </div>
