@extends('layouts.app')
@section('title', 'Laporan Murojaah')
@section('page-title', 'Laporan Murojaah')

@section('content')
<div class="space-y-6">
    <!-- Filter Section -->
    <div class="bg-white rounded-xl border border-gray-200 p-4">
        <form method="GET" action="{{ route('admin.laporan.murojaah') }}" class="flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @if(request()->anyFilled(['bulan', 'tingkatan', 'santri_id', 'ustadz_id']))
                    <a href="{{ route('admin.laporan.murojaah') }}" class="px-5 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($murojaahList as $murojaah)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3 text-gray-800">{{ \Carbon\Carbon::parse($murojaah->tanggal)->format('d M Y') }}</td>
                            <td class="px-4 py-3 font-medium text-gray-800">{{ optional($murojaah->santri)->nama ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-500">{{ optional($murojaah->santri)->tingkatan_label ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">
                                <div>Juz {{ $murojaah->juz }}</div>
                                <div class="text-xs text-gray-500">{{ $murojaah->surat }} (Ayat {{ $murojaah->ayat }})</div>
                            </td>
                            <td class="px-4 py-3 text-center font-semibold text-gray-800">{{ $murojaah->nilai }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ optional($murojaah->ustadz)->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-center">
                                @if($murojaah->status_selesai)
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-gray-400">
                                @if(request()->anyFilled(['bulan', 'tingkatan', 'santri_id', 'ustadz_id']))
                                    Tidak ada data murojaah yang sesuai dengan filter.
                                @else
                                    Belum ada data murojaah.
                                @endif
                            </td>
                        </tr>
                    @endforelse
